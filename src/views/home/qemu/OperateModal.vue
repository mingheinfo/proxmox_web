  ...
  >
    <template slot="content">
						...
						<dl>
							...
							<dd>
								...
								<m-select
									...
									@on-change="(value) => {nodename = value; checkMigratePreconditions()}"
									v-model="nodename"
									placeholder="请选择节点"
								>
									<m-option
								...
								>
									...
									>
										...
										<div class="table-tr">
											...
											<span class="table-td" :title="byteToSize(item.avail)">{{ byteToSize(item.avail) }}</span>
											<span class="table-td" :title=" byteToSize(item.total)">{{ byteToSize(item.total) }}</span>
										</div>
									</m-option>
								</m-select>

								<m-checkbox label="Force"
														v-model="migration.overwriteLocalResourceCheck"
														@change="checkMigratePreconditions(true)" 
														v-show="!setLocalResourceCheckboxHidden"></m-checkbox>
							</dd>
							<dt></dt>
								...
								<el-table :data="migration.preconditions">
									<el-table-column prop="severity" width="25px">
										<template slot-scope="scope">
											<i :class="['fa', `fa-${scope.row.severity === 'error' ? 'times error' : 'exclamation-triangle warning'}`]"></i>
										</template>
									</el-table-column>
						...
						<dl>
							...
							<dd>
								<m-select
									...
									:show-error="rules.nodename.error"
									:error-msg="rules.nodename.message"
									@on-change="(value) => {nodename = value; queryTargetStorage()}"
									v-model="nodename"
									placeholder="请选择节点"
								>
									<m-option
									...
									>
										...
										<div class="table-tr">
											<span class="table-td" :title="item.node">{{ item.node }}</span>
											<span class="table-td" :title="percentToFixed(item.mem / item.maxmem, 3)">{{
							...
							>
							   <div v-if="index === 0" class="table-tr">
								...
								<div class="table-tr">
									...
									<span class="table-td" :title="item.nodes">{{ item.nodes }}</span>
									<span class="table-td" :title="item.comment">{{ item.comment }}</span>
								</div>
							</m-option>
					</m-select>
      ...
      >
		...
		<template slot="footer">
			<m-button :disabled="!migration.possible" 
			          type="primary"
								class="create-btn"
								@on-click="confirm">{{modalType === 'migrate' ? '迁移' : (modalType === 'clone' ? '克隆' : (modalType === 'delete' ? '删除' : '添加/修改'))}}</m-button>
		</template>
  </m-dialog>
</template>

<script>
import { byteToSize, percentToFixed, stringFormat, browserLocalStorageGetItem, dateFormat } from "@libs/utils/index";
      ...
      if(this.running) {
		...
		*/
		setLocalResourceCheckboxHidden() {
		  ...
		  if (this.running || !this.migration.hasLocalResources ||
  ...
  methods: {
    ...
    confirm() {
	    ...
	    if(_this.modalType === 'migrate') {
					...
					if (_this.migration['with-local-disks'] && _this.db.qemuObj.status === 'running') {
						params.targetstorage = _this.storage;
					}
					if (_this.migration.overwriteLocalResourceCheck) {
						params['force'] = 1;
					}
					this.extraTitle = stringFormat(' ({0} ---> {1})',_this.qemu.node, params.target);
					_this.migreate(params)
							.then(res => {
								_this.showLog = true;
								_this.interval = window.setInterval(() => {
									_this.queryStatus(this.db.addClusterStatusObj.upid);
									_this.queryLog(this.db.addClusterStatusObj.node, this.db.addClusterStatusObj.upid);
								}, 3000)
							}).catch(res => {
			...
			if(this.modalType === 'ha') {
				...
				if(this.max_restart && this.max_restart !== 1) params['max_restart'] = this.max_restart;
				if(this.max_relocate && this.max_relocate !== 1) params['max_relocate'] = this.max_relocate;
    ...
    async __init__() {
      ...
      //在线的时候可选择存储
      if (this.db.qemuObj.status === "running" && this.modalType === 'migrate') {
        this.queryStorageList(this.nodename, {format: 1, content: 'images'});
			}
			//如果是ha的
			if(this.modalType === 'ha') {
				if(this.db.qemuObj && this.db.qemuObj.ha && String(this.db.qemuObj.ha.managed) === '0') {
					this.isCreate = true;
				} 
				if(!this.isCreate) {
					await _this.queryHaResource(_this.qemu.vmid, {_dc: new Date().getTime()});
    ...
    async checkQemuPreconditions(resetMigrationPossible) {
      ...
      if (_this.migreateObj.allowed_nodes) {
        ...
        if (target.length && !_this.migreateObj.allowed_nodes.includes(target)) {
		...
		*/
	    ...
	    if (_this.qemu.type === 'qemu') {
		   ...
		   _this.checkLxcPreconditions(resetMigrationPossible);
			...
			_this.nodeList = _this.nodeList.filter(item => _this.qemu.node !== item.node);
